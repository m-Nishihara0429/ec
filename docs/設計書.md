...
| 3 | 商品管理（CRUD） | 商品の登録・編集・削除・一覧。CSVファイルをアップロードしての一括登録にも対応（ProductService#importFromCsv） |
...
com.example.ec
 ...
 ├─ service/         UserService, ProductService（CSV一括登録importFromCsvを含む）, CartService, OrderService,
 │                     CategoryService, ReviewService, PasswordResetService, CouponService, FaqService,
 │                     InquiryService, PostalCodeService（郵便番号検索API(zipcloud)をRestTemplateで呼び出し、住所を取得）
 ...
 ├─ controller/admin/ AdminDashboardController, AdminProductController（CSV一括登録エンドポイントを含む）,
 │                     AdminCategoryController, AdminOrderController, AdminCouponController, AdminFaqController,
 ...
 └─ dto/             RegisterForm, CheckoutForm, ProductForm, ReviewForm, ProfileForm,
                       ...
                       FaqForm, ContactForm, ProductSort, SalesPoint, CsvImportResult（CSV一括登録の結果） 等
```
...
## 7.2 CSVファイルによる商品一括登録（ファイルI/O）

管理画面の商品管理（`/admin/products`）から、CSVファイルをアップロードして商品をまとめて登録できる。

- **フロー**: 管理者がCSVファイルを選択して送信（`admin/products.html`のフォーム、`enctype="multipart/form-data"`）→ `AdminProductController#importCsv`（`POST /admin/products/import`）→ `ProductService#importFromCsv` が `MultipartFile.getInputStream()` を `BufferedReader`（UTF-8）で1行ずつ読み込み、行ごとに検証・保存する
- **CSVの列**: `商品名,商品説明,価格,在庫数,カテゴリ名,画像URL`（1行目はヘッダー行として読み飛ばす。商品説明・カテゴリ名・画像URLは空でもよい）
- ダブルクォートで囲まれたフィールド内のカンマ・エスケープされた`""`にも対応する簡易CSVパーサーを自前で実装している（商品説明にカンマが含まれるケースを考慮）
- カテゴリ名を指定する場合は既存のカテゴリ名と完全一致している必要がある（`CategoryRepository#findByName`で検索）。誤入力によるカテゴリの意図しない乱立を防ぐため、CSV取り込み時に新規カテゴリは自動作成しない
- **1行の不備で全体を失敗させない**設計になっており、列数不足・数値変換失敗・カテゴリ名不一致などの行はその行だけスキップし、行番号と理由を記録して次の行の処理を続ける。取り込み後、登録できた件数とスキップした行の一覧（`CsvImportResult`）をフラッシュ属性で一覧画面に表示する
...
- テスト用マスター管理者アカウント: `master@example.com` / `master1234`（ROLE_MASTER。会員管理も可能な最上位ロール）
